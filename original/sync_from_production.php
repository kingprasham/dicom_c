<?php
/**
 * Sync from PRODUCTION Orthanc to LOCAL Database
 * This syncs data from the remote production Orthanc server to your local MySQL database
 *
 * Use this when: MRI sends to production server, and you want to see it on localhost
 */
...
echo "<!DOCTYPE html><html><head><title>Production Sync</title>";
...
echo "<h1>🔄 Syncing from PRODUCTION Orthanc to LOCAL Database</h1>";
echo "<pre>\n";

// PRODUCTION Orthanc Configuration (via API Gateway)
$PRODUCTION_GATEWAY = 'https://brendon-interannular-nonconnectively.ngrok-free.dev/';
$API_KEY = 'Hospital2025_DicomSecureKey_XyZ789ABC';

echo "Production Gateway: $PRODUCTION_GATEWAY\n";
echo "Local Database: " . DB_NAME . " @ " . DB_HOST . "\n\n";

function callProductionOrthanc($endpoint) {
    global $PRODUCTION_GATEWAY, $API_KEY;

    // Remove leading slash from endpoint
    $endpoint = ltrim($endpoint, '/');

    $url = $PRODUCTION_GATEWAY . 'api/orthanc/' . $endpoint;
    ...
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); // For ngrok
    ...
    if ($httpCode !== 200) {
        echo "<span class='warning'>⚠ HTTP $httpCode for $endpoint</span>\n";
        if ($error) echo "<span class='error'>  cURL Error: $error</span>\n";
        return null;
    }
...
// Get all patients from PRODUCTION
echo "=== FETCHING PATIENTS FROM PRODUCTION ===\n";
$patients = callProductionOrthanc('patients');

if (!$patients) {
    echo "<span class='error'>✗ Failed to connect to Production Orthanc!</span>\n";
    echo "<span class='warning'>Make sure:</span>\n";
    echo "  1. ngrok tunnel is running (START_NGROK_HIDDEN.vbs)\n";
    echo "  2. Python gateway is running (start_gateway_hidden.vbs)\n";
    echo "  3. API Gateway URL is correct in config\n";
    exit;
}

echo "<span class='success'>✓ Found " . count($patients) . " patients in Production Orthanc</span>\n\n";
...
foreach ($patients as $patientOrthancId) {
    $patientData = callProductionOrthanc("patients/$patientOrthancId");
    ...
    if (!$cachedPatient) {
        ...
        $patientsAdded++;
        echo "  <span class='success'>✓ Added patient to LOCAL cache</span>\n";
    } else {
        $patientsUpdated++;
    }

    // Get studies from PRODUCTION
    $studies = $patientData['Studies'] ?? [];
    echo "  Studies in Production: " . count($studies) . "\n";

    foreach ($studies as $studyOrthancId) {
        $studyData = callProductionOrthanc("studies/$studyOrthancId");
        ...
        foreach ($studyData['Series'] ?? [] as $seriesId) {
            $seriesData = callProductionOrthanc("series/$seriesId");
            if ($seriesData) {
        ...
        if (isset($studyData['Series']) && count($studyData['Series']) > 0) {
            $firstSeries = callProductionOrthanc("series/" . $studyData['Series'][0]);
            if ($firstSeries) {
        ...
        // Check if study exists in LOCAL database
...
echo "\n=== UPDATING PATIENT STUDY COUNTS ===\n";
$updateQuery = "UPDATE cached_patients cp
                SET study_count = (
                    SELECT COUNT(*) FROM cached_studies cs
                    WHERE cs.patient_id = cp.patient_id
                ),
                last_study_date = (
                    SELECT MAX(study_date) FROM cached_studies cs
                    WHERE cs.patient_id = cp.patient_id
                )";
$mysqli->query($updateQuery);
echo "<span class='success'>✓ Updated patient study counts</span>\n";

echo "\n=== SYNC COMPLETE ===\n";
echo "<span class='success'>Patients Added: $patientsAdded</span>\n";
echo "<span class='info'>Patients Updated: $patientsUpdated</span>\n";
...
echo "\nTotal studies in LOCAL database: <strong>{$row['count']}</strong>\n";

$result = $mysqli->query("SELECT COUNT(*) as count FROM cached_patients");
$row = $result->fetch_assoc();
echo "Total patients in LOCAL database: <strong>{$row['count']}</strong>\n";

echo "\n<span class='success'>✓✓✓ SYNC SUCCESSFUL ✓✓✓</span>\n";
echo "\n<a href='pages/patients.html' style='color:#0af;'>→ Go to Patients Page</a>\n";
echo "<a href='sync_from_production.php' style='color:#0af;margin-left:20px;'>↻ Sync Again</a>\n";
echo "<a href='sync_orthanc.php' style='color:#0af;margin-left:20px;'>🔄 Sync from Local Orthanc</a>\n";
